...
The assessment has a dedicated HubSpot form. No automated report email workflow is configured in this repository; do not promise an automatic email. The full report opens on the same page at `/erp-assessment/#report` only after the API accepts the form submission; it can then be viewed, printed or saved as PDF. The summary and three verdict labels remain ungated.

The explicit “Get my report” form uses the dedicated ERP Assessment form (`7bc569f0-a87b-487d-aca2-9adf7988bac4`, portal `9456893`, region `na1`). Its public published definition was verified on 2026-09-07: firstname, lastname, email, company, message. The website requires the four contact fields and submits the report through hidden `message`. The proposed `preferred_contact_method` and `lead_source_detail` properties are absent from this published form, so do not send them as separate fields. The message includes `Source: ERP Assessment: Full report request`, `Preferred contact: Email`, the complete assessment, version and labelled answers. The dedicated form ID distinguishes these leads from other assets. Find the assessment in the contact’s corresponding form-submission activity, rather than relying on the latest message contact property (which later enquiries may update). Configure assessment notifications/workflows on this form independently of the strategy-call form. No answer payload is sent during the assessment. The gate explicitly explains answer storage and follow-up.
...
To enable automated email reports later: configure and verify report email delivery for each outcome using available HubSpot capabilities and the dedicated assessment form, then update the website CTA. The browser must not contain secret credentials. Form submission success is not evidence of email delivery.
...
Local browser acceptance uses stubbed HubSpot success/error responses and verifies the outgoing schema and dedicated form endpoint. A real authorised report request should confirm that HubSpot accepts the message and the team receives it. No production form submissions are made by automated tests.
